...
######################################
    def occ( self ):
        #number of times that this graph appears in the fixed locus
        parts = [ self.vertex_colors[k] for k in self.vertex_colors ]
        return super().automorphism_group().order()/super().automorphism_group( edge_labels = True, partition = parts).order()
        
    def c_top( self ):
        return (self.function_V()*self.function_E())^-1
    
    def a( self ):
        #the constant a(\Gamma)
        parts = [ self.vertex_colors[k] for k in self.vertex_colors ]
        b = super().automorphism_group( edge_labels = True, partition = parts).order()
        for e in self.edges():
            b *= e[2]
        return b
    ...
    def altern( self ):
        #the product occ()*a()
        b = super().automorphism_group().order()
        for e in self.edges():
            b *= e[2]
        return b
    ...
    def quintic(self):
        ...
        lam = lambda v:  gens[vcols[v]]
        for e in self.edges():
            for alph in list(IntegerRange(0,5*e[2]+1)):
                p1 *= (alph*lam(e[0]) + (5*e[2]-alph) * lam(e[1]))/e[2]
        for v in self.vertices():
            q1 *= (5*lam(v))^(1-len(self.neighbors(v)))        
        return p1*q1
    ...
    def contact( self ):       
        ...
        lam = lambda v:  gens[vcols[v]]        
        for e in self.edges():
            for alph in list(IntegerRange(1,2*e[2])):
                p1 *= (alph*lam(e[0]) + (2*e[2]-alph) * lam(e[1]))/e[2]
        for v in self.vertices():
            q1 *= (2*lam(v))^(len(self.neighbors(v))-1)        
        return p1*q1
...
#        vcols = [ self.vertex_color( x ) for x in self.vertices()]
#        s = sum([ e[2]*sum([ t*lam(e[0])+(a-t)*lam(e[1]) for t in [0..a-1]]) 
#                    for e in self.edges()])
